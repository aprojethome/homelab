...
- Déployé via `docker compose up -d`
- Image : `prom/node-exporter:latest`
- Expose les métriques système (CPU, RAM, disque, réseau) du Raspberry Pi sur le port `9100`
- ⚠️ Penser à retirer l'attribut `version` du `compose.yaml` (obsolète, ignoré par Docker Compose)
...
- Déployé via `docker compose up -d`
- Scrape config ciblant `node-exporter:9100`
- Target `raspberry` confirmée **UP** dans l'interface Prometheus (`/targets`)
...
- **Type** : HTTP(s)
- **URL** : `http://pihole/admin/` — attention, **HTTP et non HTTPS** : Pi-hole sert son interface admin en HTTP par défaut. Utiliser `https://` sur une interface sans certificat provoque un statut "offline" alors que le service tourne normalement.
